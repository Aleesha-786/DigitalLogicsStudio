...
const Boolforge = ({
  ...
  const computeGateOutput = (gate, inputs, outputIndex = 0) => {
    ...
    switch (gate.type) {
      case "INPUT": return gate.inputValues[0] || false;
      case "AND": {
        const n = gate.inputs || 2;
        let allHigh = true;
        for (let i = 0; i < n; i++) if (!(inputs[i] ?? false)) { allHigh = false; break; }
        return allHigh;
      }
      case "OR": return ci.some(Boolean);
      case "NOT": return inputs[0] !== undefined ? !inputs[0] : false;
      case "NAND": {
        const n = gate.inputs || 2;
        let allHigh = true;
        for (let i = 0; i < n; i++) if (!(inputs[i] ?? false)) { allHigh = false; break; }
      ...
      case "NOR": return !ci.some(Boolean);
      case "XOR": return ci.length >= 2 && ci.reduce((acc, v) => acc !== v, false);
      case "XNOR": return ci.length >= 2 && !ci.reduce((acc, v) => acc !== v, false);
      case "BUFFER":
      ...
      case "MUX4": {
        const s0 = inputs[4] ?? false, s1 = inputs[5] ?? false;
        const sel = (s1 ? 2 : 0) + (s0 ? 1 : 0);
        return inputs[sel] ?? false;
      }
      case "MUX8": {
        const s0 = inputs[8] ?? false, s1 = inputs[9] ?? false, s2 = inputs[10] ?? false;
        const sel = (s2 ? 4 : 0) + (s1 ? 2 : 0) + (s0 ? 1 : 0);
        return inputs[sel] ?? false;
      }
      case "DEMUX2": {
        const d = inputs[0] ?? false, s = inputs[1] ?? false;
        if (outputIndex === 0) return !s && d;
        if (outputIndex === 1) return s && d;
        return false;
      }
      case "DEMUX4": {
        const d = inputs[0] ?? false, s0 = inputs[1] ?? false, s1 = inputs[2] ?? false;
        const sel = (s1 ? 2 : 0) + (s0 ? 1 : 0);
        return sel === outputIndex && d;
      }
      case "DEMUX8": {
        const d = inputs[0] ?? false, s0 = inputs[1] ?? false, s1 = inputs[2] ?? false, s2 = inputs[3] ?? false;
        const sel = (s2 ? 4 : 0) + (s1 ? 2 : 0) + (s0 ? 1 : 0);
        return sel === outputIndex && d;
      }
      case "ENC4": {
      ...
      case "HALF_ADDER": {
        const a = inputs[0] ?? false, b = inputs[1] ?? false;
        return outputIndex === 0 ? (a !== b) : (a && b);
      }
      case "FULL_ADDER": {
        const a = inputs[0] ?? false, b = inputs[1] ?? false, cin = inputs[2] ?? false;
        const sum = (a !== b) !== cin;
      ...
      case "FULL_SUBTRACTOR": {
        const a = inputs[0] ?? false, b = inputs[1] ?? false, bin = inputs[2] ?? false;
        const diff = (a !== b) !== bin;
  ...
  const evaluateGate = useCallback((gate, outputIndex = 0) => {
  ...
  const deleteGate = useCallback((gateOrId = null) => {
    let targets = [];
    if (gateOrId) {
    ...
    setWires((prev) => prev.filter((w) => !targets.includes(w.fromId) && !targets.includes(w.toId)));
  ...
  const addGate = (type) => {
    ...
    const newGate = {
      id: gateIdCounter, type, label,
      x: 30 + col * GATE_STEP_X, y: 30 + row * GATE_STEP_Y,
      inputs: finalInputs,
  ...
  const addInputSlot = useCallback((e, gate) => {
    e.stopPropagation();
    if (!MULTI_INPUT_GATES.has(gate.type) || gate.inputs >= MAX_GATE_INPUTS) return;
    setGates((prev) => prev.map((g) => g.id === gate.id ? { ...g, inputs: g.inputs + 1 } : g));
    saveToHistory();
  }, [saveToHistory]);

  const removeInputSlot = useCallback((e, gate) => {
    e.stopPropagation();
    if (!MULTI_INPUT_GATES.has(gate.type) || gate.inputs <= MIN_GATE_INPUTS) return;
    const lastIdx = gate.inputs - 1;
    setWires((prev) => prev.filter((w) => !(w.toId === gate.id && w.toIndex === lastIdx)));
    setGates((prev) => prev.map((g) => g.id === gate.id ? { ...g, inputs: g.inputs - 1 } : g));
    saveToHistory();
  }, [saveToHistory]);
  ...
  const commitRename = () => {
    ...
    if (trimmed) {
      setGates((prev) => prev.map((g) => g.id === renamingGate.id ? { ...g, label: trimmed } : g));
      saveToHistory();
    }
    setRenamingGate(null);
    setRenameValue("");
  };
  const cancelRename = () => { setRenamingGate(null); setRenameValue(""); };

  // ── Toggle input ───────────────────────────────────────────────────────────
  const toggleInput = (gate) => {
    setGates((prev) => prev.map((g) => g.id === gate.id ? { ...g, inputValues: [!g.inputValues[0]] } : g));
  };
  ...
  const startDrag = (e, gate) => {
    ...
    if (isCtrl) {
      if (selectedGateIds.includes(gate.id)) nextSelection = nextSelection.filter(id => id !== gate.id);
    ...
    gates.forEach((g) => { if (nextSelection.includes(g.id)) startPositions[g.id] = { x: g.x, y: g.y }; });
    ...
    setGates((prev) =>
      prev.map((g) => {
        if (selectedGateIds.includes(g.id)) {
          ...
          if (startPos) {
            return { ...g, x: snapToGrid(startPos.x + dx), y: snapToGrid(startPos.y + dy) };
          }
        }
        return g;
      })
  ...
  const handleCanvasContextMenu = (e) => {
    ...
    for (const wire of wires) {
      ...
      const cp1x = fromX + controlDistance, cp1y = fromY;
      const cp2x = toX - controlDistance, cp2y = toY;

      const SAMPLES = 60, HIT_RADIUS = 8;
      for (let i = 0; i <= SAMPLES; i++) {
        const t = i / SAMPLES, mt = 1 - t;
        const bx = mt ** 3 * fromX + 3 * mt ** 2 * t * cp1x + 3 * mt * t ** 2 * cp2x + t ** 3 * toX;
        const by = mt ** 3 * fromY + 3 * mt ** 2 * t * cp1y + 3 * mt * t ** 2 * cp2y + t ** 3 * toY;
    ...
    if (e.target === canvasRef.current) {
      ...
      if (spacePressed || isMiddleClick) {
        ...
        if (!selectionToolActive && !e.shiftKey) {
          ...
          if (!isCtrl) { setSelectedGateIds([]); setSelectedGate(null); }
        }
      }
    }
  };
  ...
  const handleTouchEnd = useCallback(() => {
    if (touchStateRef.current.type === "drag" && dragging) { setDragging(false); saveToHistory(); }
  ...
  const copySelectedGates = useCallback(() => {
    ...
    copiedDataRef.current = {
      ...
      wires: JSON.parse(JSON.stringify(wires.filter(
  ...
  const pasteGates = useCallback(() => {
    ...
    const pastedGates = copiedGates.map((g) => {
      ...
      return { ...g, id: newId, label: newLabel, x: g.x + 40, y: g.y + 40, inputValues: g.type === "INPUT" ? [false] : [] };
    });

    const pastedWires = copiedWires.map((w) => ({ ...w, id: currentWireId++, fromId: idMap[w.fromId], toId: idMap[w.toId] }));
  ...
  const duplicateSelectedGates = useCallback(() => {
    ...
    const duplicatedWires = selectedWiresList.map((w) => ({ ...w, id: currentWireId++, fromId: idMap[w.fromId], toId: idMap[w.toId] }));
  ...
  const clearCircuit = () => {
    setGates([]); setWires([]);
    setGateIdCounter(0); setWireIdCounter(0);
    setInputCounter(0); setOutputCounter(0);
    setHistory([]); setHistoryIndex(-1);
  };

  // ── Truth table / expression helpers ───────────────────────────────────────
  const evaluateGateWithGates = useCallback((gate, gatesArray, outputIndex = 0) => {
    const localGateMap = new Map();
    gatesArray.forEach((g) => localGateMap.set(g.id, g));
    const incomingWires = new Map();
    gatesArray.forEach((g) => incomingWires.set(g.id, []));
    wires.forEach((w) => { if (incomingWires.has(w.toId)) incomingWires.get(w.toId).push(w); });
    ...
    for (let iter = 0; iter < 100; iter++) {
      ...
      for (const g of gatesArray) {
        ...
        if (IC_TYPES.has(g.type)) {
          ...
          if (!Array.isArray(oldVals) || newVals.some((v, i) => v !== oldVals[i])) { next.set(g.id, newVals); changed = true; }
        } else {
  ...
  const deriveExpression = useCallback((gate, gatesArray, depth = 0, visited = new Set()) => {
    ...
    switch (gate.type) {
      case "OUTPUT": case "BUFFER": return slots[0];
  ...
  const applyGeneratedCircuit = useCallback((data) => {
  ...
  const runAiGenerate = useCallback(async (description, sendCurrentCircuit) => {
      ...
      if (circuit.gates && circuit.gates.length > 0) {
        ...
        setTimeout(() => {
          setHistory([{
            ...
            inputCounter: inputCount, outputCounter: outputCount,
          }]);
          setHistoryIndex(0);
        }, 100);
      }
  ...
  const drawWires = useCallback(() => {
    ...
    wires.forEach((wire) => {
      ...
      const distance = Math.sqrt(dx * dx + dy * dy);
      const controlDistance = Math.min(Math.abs(dx) / 2, distance / 3);
      ctx.bezierCurveTo(fromX + controlDistance, fromY, toX - controlDistance, toY, toX, toY);
      ctx.stroke();
      ctx.shadowBlur = 0;
    });
    ctx.restore();
  ...
  useEffect(() => {
    ...
    const resizeCanvas = () => {
      ...
      resizeTimeout = setTimeout(() => {
        ...
        if (w > 0 && h > 0) { canvas.width = w; canvas.height = h; drawWires(); }
      }, 100);
    };
    resizeCanvas();
  ...
  useEffect(() => {
    const down = (e) => { if (e.key === " " && document.activeElement.tagName !== "INPUT" && document.activeElement.tagName !== "TEXTAREA") setSpacePressed(true); };
  ...
  useEffect(() => {
    const handler = (e) => {
      ...
      else if (e.ctrlKey && e.key === "a") { e.preventDefault(); setSelectedGateIds(gates.map(g => g.id)); }
      else if (e.ctrlKey && e.key === "c") { e.preventDefault(); copySelectedGates(); }
      else if (e.ctrlKey && e.key === "v") { e.preventDefault(); pasteGates(); }
      else if (e.ctrlKey && e.key === "d") { e.preventDefault(); duplicateSelectedGates(); }
      else if ((e.key === "Delete" || e.key === "Backspace") && selectedGateIds.length > 0) { e.preventDefault(); deleteGate(); }
      else if (e.key === "Escape") { setConnectingFrom(null); setSelectedGateIds([]); }
    };
  ...
  const circuitTool = (
    <div
      ...
      onMouseMove={(e) => { if (isPanning || isSelecting) handleMouseMove(e); else onDrag(e); }}
      onMouseUp={() => { stopDrag(); handleMouseUp(); }}
      onTouchMove={(e) => { if (e.touches.length === 1) { const t = e.touches[0]; if (isPanning) handleMouseMove(t); else onDrag(t); } }}
      onTouchEnd={() => { stopDrag(); handleMouseUp(); }}
    >
      {/* Sidebar */}
      <div className="sidebar">
        <h2>Circuit Forge</h2>
        <button onClick={() => setSelectionToolActive(v => !v)} className={`toggle-selection-btn${selectionToolActive ? " active" : ""}`}>
        ...
        <div className="palette-section">
          ...
          <div className="gate-palette">
            {["INPUT","OUTPUT","AND","OR","NOT","NAND","NOR","XOR","XNOR","BUFFER"].map(type => (
              <button key={type} className="gate-btn" onClick={() => addGate(type)}>{type}</button>
            ))}
          </div>
        </div>
        <div className="palette-section">
          ...
          <div className="gate-palette">
            {[{ type: "MUX2", label: "MUX 2:1" }, { type: "MUX4", label: "MUX 4:1" }, { type: "MUX8", label: "MUX 8:1" }].map(({ type, label }) => (
              <button key={type} className="gate-btn gate-btn--ic" onClick={() => addGate(type)}>{label}</button>
            ))}
          </div>
        </div>
        <div className="palette-section">
          ...
          <div className="gate-palette">
            {[{ type: "DEMUX2", label: "DEMUX 1:2" }, { type: "DEMUX4", label: "DEMUX 1:4" }, { type: "DEMUX8", label: "DEMUX 1:8" }].map(({ type, label }) => (
              <button key={type} className="gate-btn gate-btn--ic" onClick={() => addGate(type)}>{label}</button>
            ))}
          </div>
        </div>
        <div className="palette-section">
          ...
          <div className="gate-palette">
            {[{ type: "ENC4", label: "ENC 4:2" }, { type: "ENC8", label: "ENC 8:3" }].map(({ type, label }) => (
              <button key={type} className="gate-btn gate-btn--ic" onClick={() => addGate(type)}>{label}</button>
            ))}
          </div>
        </div>
        <div className="palette-section">
          ...
          <div className="gate-palette">
            {[{ type: "DEC4", label: "DEC 2:4" }, { type: "DEC8", label: "DEC 3:8" }].map(({ type, label }) => (
              <button key={type} className="gate-btn gate-btn--ic" onClick={() => addGate(type)}>{label}</button>
            ))}
          </div>
        </div>
        <div className="palette-section">
          ...
          <div className="gate-palette">
            {[{ type: "HALF_ADDER", label: "Half Adder" }, { type: "FULL_ADDER", label: "Full Adder" }, { type: "ADD4", label: "4 bit Adder" }, { type: "CLADD4", label: "Carry LA 4" }].map(({ type, label }) => (
              <button key={type} className="gate-btn gate-btn--ic" onClick={() => addGate(type)}>{label}</button>
            ))}
          </div>
        </div>
        <div className="palette-section">
          ...
          <div className="gate-palette">
            {[{ type: "HALF_SUBTRACTOR", label: "Half Subtractor" }, { type: "FULL_SUBTRACTOR", label: "Full Subtractor" }].map(({ type, label }) => (
              <button key={type} className="gate-btn gate-btn--ic" onClick={() => addGate(type)}>{label}</button>
            ))}
          </div>
        </div>
        ...
        <div className="instructions">
          ...
          <p>• Enable <strong>⬚ Selection Tool</strong> to box-select components</p>
      ...
      <div className="canvas-container" ref={containerRef}>
        ...
        <div className="gates-container" style={{ transform: `translate(${panOffset.x}px, ${panOffset.y}px) scale(${zoom})` }}>
          {isSelecting && (
            <div className="selection-rectangle" style={{
              ...
              pointerEvents: "none", zIndex: 1000, borderRadius: "3px",
            ...
            return (
              ...
              >
                ...
                {isIC && Array.from({ length: icMeta.outputs }).map((_, outIdx) => {
                  ...
                  return (
                    <div key={`out-${outIdx}`} className={`connection-point output-point ic-output-point ${isConnecting ? "active" : ""} ${evaluateGate(gate, outIdx) ? "ic-output-point--high" : ""}`}
                      style={{ top: `${topPct}%` }} title={icMeta.outputLabels[outIdx]} onClick={() => startConnection(gate, outIdx)}>
                ...
                {/* Standard output */}
                {!isIC && gate.hasOutput && (
                  <div className={`connection-point output-point ${cfGateId === gate.id ? "active" : ""}`} onClick={() => startConnection(gate, 0)} />
                )}
                ...
                {isIC && Array.from({ length: icMeta.inputs }).map((_, idx) => {
                  ...
                  return (
                    <div key={`in-${idx}`} className={`connection-point input-point ic-input-point ${connectingFrom ? "active" : ""}`}
                      style={{ top: `${topPct}%` }} title={icMeta.inputLabels[idx]} onClick={() => completeConnection(gate, idx)}>
                ...
                {!isIC && gate.inputs >= 2 && Array.from({ length: gate.inputs }).map((_, idx) => {
                  ...
                  return (
                    <div key={idx} className={`connection-point input-point ${connectingFrom ? "active" : ""}`}
                      style={{ top: `${topPct}%` }} onClick={() => completeConnection(gate, idx)} />
                  );
                })}
                {!isIC && gate.inputs === 1 && (
                  <div className={`connection-point input-point ${connectingFrom ? "active" : ""}`}
                    style={{ top: "50%" }} onClick={() => completeConnection(gate, 0)} />
                )}
              </div>
            );
          })}
        ...
        <div className="canvas-overlay-controls">
          <button className={`canvas-overlay-btn${selectionToolActive ? " canvas-overlay-btn--active" : ""}`}
            onClick={() => setSelectionToolActive(v => !v)}
            style={selectionToolActive ? { background: "var(--accent-primary, #7c3aed)", color: "#fff", borderColor: "var(--accent-primary, #7c3aed)" } : {}}
          >⬚</button>
      ...
      <div className="truth-table-panel">
        ...
        {!embedded && (
          <div className="ai-assistant-section">
            ...
            <textarea className="ai-textarea" value={aiPrompt} onChange={(e) => setAiPrompt(e.target.value)}
              placeholder="Describe the circuit (e.g. 'half adder', 'A AND B OR C')…" rows={2} />
            <div className="controls">
              <button className="btn hint-btn" onClick={handleRequestHint} disabled={hintLoading}
                style={{ cursor: hintLoading ? "wait" : "pointer" }}>
                {hintLoading ? "💡 Thinking…" : "💡 Get Hint"}
              </button>
              <button className="btn generate-btn" onClick={handleGenerateCircuit} disabled={isGenLoading}
          ...
          <div className="input-controls">
            ...
            {inputGates.map(gate => (
              <div key={gate.id} className="input-toggle">
                ...
                <div className={`toggle-btn ${gate.inputValues[0] ? "on" : ""}`} onClick={() => toggleInput(gate)} />
              </div>
            ))}
          </div>
        )}
        ...
        <div className="controls">
          ...
          <SaveAndLoad
            data={{ gates, wires, gateIdCounter, wireIdCounter, inputCounter, outputCounter }}
            setGates={setGates} setWires={setWires} setGateIdCounter={setGateIdCounter}
            setWireIdCounter={setWireIdCounter} setInputCounter={setInputCounter}
            setOutputCounter={setOutputCounter} saveToHistory={saveToHistory}
          />
        ...
        <div className="modal-overlay" onClick={cancelRename}>
          <div className="rename-dialog" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="rename-text">
              Enter a custom label for this <strong className="gate-type">{renamingGate.type}</strong> gate.
            </p>
            ...
            <div className="rename-actions">
              <button className="btn cancel-btn" onClick={cancelRename}>Cancel</button>
              <button className="btn rename-btn" onClick={commitRename}>Rename</button>
            </div>
          </div>
        </div>
      )}
  ...
  return (
    <div className={`boolforge-page theme-${theme}`}>
      ...
      {navbarVisible && (
        <Navbar toggleTheme={toggleTheme} theme={theme} onToggleNavbar={() => setNavbarVisible(false)} />
      )}
      {!navbarVisible && (
